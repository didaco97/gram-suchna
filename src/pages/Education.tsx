...
import { fetchEducationOpportunities } from '@/api/perplexityApi';
...
const parseSchemes = (rawText: string): SchemeItem[] => {
  try {
    ...
    return matches.map(match => {
      ...
      const title = titleMatch ? titleMatch[1].trim() : "Education Opportunity";
    ...
    console.error("Error parsing education opportunities:", error);
...
const Education = () => {
  ...
  const fetchOpportunities = async () => {
    ...
    try {
      ...
      if (!apiKey) {
        setError("Please add your Perplexity API key to fetch education opportunities.");
        setLoading(false);
        return;
      }
      
      const rawData = await fetchEducationOpportunities();
      const parsedSchemes = parseSchemes(rawData);
      setSchemes(parsedSchemes);
    } catch (err) {
      console.error("Error fetching education opportunities:", err);
      setError(err instanceof Error ? err.message : "Failed to fetch education opportunities");
    } finally {
      setLoading(false);
    }
  };
  ...
  return (
    <div className="min-h-screen flex flex-col">
      ...
      <main className="flex-grow container mx-auto px-4 py-8">
        ...
        <div className="mb-6">
          <LocationInput onSave={() => fetchOpportunities()} />
        </div>
